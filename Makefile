...
PANDOCARGS_.tex=-s
PANDOCMD=mkdir -p $(dir $@) ; $(FIXLINKS) $< | pandoc -f markdown_github-hard_line_breaks+markdown_in_html_blocks+raw_tex --latex-engine=xelatex --email-obfuscation=none -V author=Databrary -V date="$(GITDATE)" -V geometry="margin=.75in" -V links-as-notes $(PANDOCARGS_$(suffix $@)) -o $(GEN)
